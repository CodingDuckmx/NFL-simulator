...
class Game:
    ...
    #         self.touchdown(self.teams[i])
        if list(self.score.values())[0] == list(self.score.values())[1]:
            self.tying_teams_ = [list(self.score.keys())[0],list(self.score.keys())[1]]
        else:
            self.tying_teams_ = []
        ...
        return self.winning_team_, self.losing_team_, self.tying_teams_ 
