...
######### README ##########
'''
This file intents to simulate the matches.
'''
###########################
...
matches_other_division_afc1 = list(product(afc_east,afc_west))
for match in matches_other_division_afc1:
    matches_other_division.append(match)

matches_other_division_afc2 = list(product(afc_north,afc_south))
for match in matches_other_division_afc2:
    matches_other_division.append(match)

matches_other_division_nfc1 = list(product(nfc_east,nfc_west))
for match in matches_other_division_nfc1:
    matches_other_division.append(match)

matches_other_division_nfc2 = list(product(nfc_north,nfc_south))
for match in matches_other_division_nfc2:
    matches_other_division.append(match)

# Matches with a division of the opposite conference:

matches_other_conference_1 = list(product(afc_east,nfc_west))
for match in matches_other_conference_1:
    matches_other_division.append(match)

matches_other_conference_2 = list(product(afc_north,nfc_east))
for match in matches_other_conference_2:
    matches_other_division.append(match)

matches_other_conference_3 = list(product(afc_south,nfc_north))
for match in matches_other_conference_3:
    matches_other_division.append(match)

matches_other_conference_4 = list(product(afc_west,nfc_south))
...
# Interconference games determined by last year's standings. dbly = determined by last year
# In this case, as we don't have previous history, I'm matching them by order.

for i in list(range(4)):
    matches_dbly.append((division_list_afc[0][i],division_list_afc[1][i]))
    matches_dbly.append((division_list_afc[0][i],division_list_afc[2][i]))
    matches_dbly.append((division_list_nfc[0][i],division_list_nfc[2][i]))
